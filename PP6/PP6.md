...
<img src="https://github.com/xiao10ma/Parallel-Programming/blob/master/PP6/vis.jpg?raw=true" style="zoom:33%;" />

### 4. 实验感想

在这次实验中，我通过使用Pthreads库实现了并行计算，特别是将OpenMP的并行循环结构替换为基于Pthreads的自定义parallel_for函数。通过这种方式，我们能够手动控制线程的创建、执行和同步，而不是依赖OpenMP的自动管理。这为深入理解并行计算的底层细节提供了极好的机会。
